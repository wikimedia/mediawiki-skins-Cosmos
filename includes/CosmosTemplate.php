...
class CosmosTemplate extends BaseTemplate {
		    ...
		    global $wgManageWikiForceSidebarLinks, $wgManageWikiSidebarLinks, $wgManageWiki;
		       ...
		       foreach ( (array)ManageWiki::listModules() as $module ) {
		        ...
		        if ($this->getMsg( 'cosmos-navigation-explore-tab' )->text() !== '-' && $this->getMsg( 'cosmos-navigation-explore-tab' )->text() !== ''){
		            ...
		            $html .= Html::rawElement( 'li', [ 'class' => 'wds-tabs__tab' ], str_replace('<li>', "<li class='wds-tabs__tab'>", '<div class="wds-dropdown"><div class="wds-tabs__tab-label wds-dropdown__toggle">' . Icon::getIcon( 'explore' )->makeSvg( 91, 91, [ 'id' => 'cosmos-icons-explore', 'class' => 'wds-icon' ] ) . '<span style="padding-top: 2px;">' . $this->getMsg( 'cosmos-explore' )->text() . '</span>' . Icon::getIcon( 'dropdown' )->makeSvg( 14, 14, [ 'id' => '', 'class' => 'wds-dropdown__toggle-chevron' ] ) . '</div><div class="wds-is-not-scrollable wds-dropdown__content"><ul class="wds-list wds-is-linked wds-has-bolded-items">' . $exploreTab));
		        }
		        $html .= Html::closeElement( 'ul' );
	...
	protected function buildSearchBar( string &$html, Config $config ) : void {
		...
		protected function buildWikiHeader(string &$html, Config $config) {
		        ...
		        $html .= Html::rawElement( 'a', [ 'class' => 'wds-button wds-is-secondary createpage', 'id' => 'createpage', 'href' => '#create-article', 'data-tracking' => 'add-new-page', 'title' => $this->getMsg( 'Cosmos-add-new-page-title')->text() ], '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1000 1000" class="wds-icon wds-icon-small" id="wds-icons-page-small"' . ($permissionManager->userHasRight( $skin->getUser(), 'adminlinks' ) ? ' style="margin-right: 0;"' : '' ) . '><path d="M745,440.6V181L573.9,10H10v980h735v-1.8c137.8-15.3,244.9-132,245-273.8C989.9,572.5,882.8,455.8,745,440.6z M561.2,83.9l109.9,109.9H561.2C561.2,193.8,561.2,83.9,561.2,83.9z M71.3,928.8V71.2H500V255h183.8v185.6c-137.8,15.3-245,131.9-245,273.8c0,86.6,40.1,163.9,102.6,214.4H71.3z M714.4,925.2c-116.4-0.3-210.6-94.4-210.9-210.8C503.8,598,598,503.8,714.4,503.5c116.3,0.3,210.5,94.4,210.8,210.9C924.9,830.7,830.7,924.9,714.4,925.2z"></path><polygon points="745.1,683.8 745.1,561.3 683.8,561.3 683.8,683.8 561.3,683.8 561.3,745 683.8,745 683.8,867.5 745.1,867.5 745.1,745 867.5,745 867.5,683.8 "></polygon></svg>' . (!$permissionManager->userHasRight( $skin->getUser(), 'adminlinks' ) ? (empty( $this->data["username"]) ? $this->getMsg( "Cosmos-anon-add-new-page-text")->escaped() : $this->getMsg( "Cosmos-add-new-page-text")->escaped()) : '' ));
		       	if(!empty( $this->data["username"])){
		           	$html .= Html::rawElement( 'a', [ 'class' => 'wds-button wds-is-secondary', 'href' => htmlspecialchars( Title::newFromText('RecentChanges', NS_SPECIAL)->getFullURL()), 'data-tracking' => 'recent-changes', 'title' => $this->getMsg( 'Cosmos-recentchanges' )->text() ], Icon::getIcon( 'recentchanges' )->makeSvg( 22, 22, [ 'class' => 'wds-icon-small', 'id' => 'wds-icons-activity-small', 'stroke' => 'currentColor','stroke-linecap' => 'round', 'stroke-linejoin' => 'round', 'stroke-width' => 2 ] ) );
		        if ( !empty( $this->data['username'] ) && $permissionManager->userHasRight( $skin->getUser(), 'adminlinks' )) {
		        	$html .= Html::rawElement( 'a', [ 'class' => 'wds-button wds-is-secondary', 'href' => htmlspecialchars( Title::newFromText('AdminLinks', NS_SPECIAL)->getFullURL()), 'data-tracking' => 'admin-links', 'title' => $this->getMsg( 'Cosmos-adminlinks' )->text() ], '<svg xmlns="http://www.w3.org/2000/svg" class="wds-icon-small" id="wds-icons-dashboard-small" viewBox="0 0 24 24"><defs><style>.a,.b{fill:none;stroke:currentcolor;stroke-linecap:round;}.a{stroke-miterlimit:10;}.b{stroke-linejoin:round;}</style></defs><circle class="a" cx="12" cy="12" r="11.5"/><path class="b" d="M13.414,13.414a2,2,0,0,1-2.828-2.828c.781-.781,8.132-5.3,8.132-5.3S14.2,12.633,13.414,13.414Z"/><line class="a" x1="3.5" y1="12" x2="5" y2="12"/><line class="a" x1="5.99" y1="5.99" x2="7.05" y2="7.05"/><line class="a" x1="12" y1="3.5" x2="12" y2="5"/><line class="a" x1="20.5" y1="12" x2="19" y2="12"/><path class="a" d="M20.633,19.6A14.708,14.708,0,0,0,12,17a14.708,14.708,0,0,0-8.633,2.6"/></svg>' );
		        }
		      	    $html .= Html::rawElement( 'div', [ 'class' => 'wds-dropdown' ], '<div class="wds-button wds-is-secondary wds-dropdown__toggle"><svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 384.97 384.97" class="wds-icon wds-icon-small" id="wds-icons-more"><g><path d="M192.485,96.242c26.575,0,48.121-21.546,48.121-48.121S219.06,0,192.485,0s-48.121,21.546-48.121,48.121S165.91,96.242,192.485,96.242z M192.485,24.061c13.293,0,24.061,10.779,24.061,24.061s-10.767,24.061-24.061,24.061s-24.061-10.767-24.061-24.061S179.191,24.061,192.485,24.061z"></path><path d="M192.485,288.727c-26.575,0-48.121,21.546-48.121,48.121c0,26.575,21.546,48.121,48.121,48.121s48.121-21.546,48.121-48.121C240.606,310.274,219.06,288.727,192.485,288.727z M192.485,360.909c-13.293,0-24.061-10.767-24.061-24.061s10.767-24.061,24.061-24.061s24.061,10.779,24.061,24.061S205.778,360.909,192.485,360.909z"></path><path d="M192.485,144.364c-26.575,0-48.121,21.546-48.121,48.121s21.546,48.121,48.121,48.121s48.121-21.546,48.121-48.121S219.06,144.364,192.485,144.364z M192.485,216.545c-13.293,0-24.061-10.767-24.061-24.061s10.767-24.061,24.061-24.061s24.061,10.779,24.061,24.061C216.545,205.766,205.778,216.545,192.485,216.545z"></path></g></svg></div><div class="wds-dropdown__content wds-is-not-scrollable wds-is-right-aligned"><ul class="wds-list wds-is-linked"><li><a href="'. htmlspecialchars( Title::newFromText("Upload", NS_SPECIAL)->getFullURL()) . '" data-tracking="more-upload-file">' . $this->getMsg( 'cosmos-upload' )->text() . '</a></li><li><a href="' . htmlspecialchars( Title::newFromText('RecentChanges', NS_SPECIAL)->getFullURL()) . '" data-tracking="more-recent-changes">' . $this->getMsg( 'Cosmos-recentchanges' )->text() . '</a></li></ul></div>' );
		       	}
		        $html .= Html::closeElement( 'div' );
		        $html .= Html::closeElement( 'div' );
